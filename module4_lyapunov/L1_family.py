...
def y_cross(t,state):
    if t<0.5: return 1.0
    return state[1]
y_cross.terminal=False;y_cross.direction=-1

def single_shooter(x0,ydot0,tol=1e-12,max_iter=50):
    for i in range(max_iter):
        Y0=np.array([x0,0,0,0,ydot0,0])
        state0=np.concatenate([Y0,np.eye(6).flatten()])
        sol=solve_ivp(cr3bp_stm,[0,20],state0,events=y_cross,
...
cases=[(0.820,0.51),(0.800,0.53),(0.780,0.55),(0.760,0.59),(0.740,0.55),(0.720,0.65),(0.700,0.69)]
L1x=0.83691513

fig,ax=plt.subplots(figsize=(8,8))
colors=plt.cm.Blues(np.linspace(0.35,0.95,len(cases)))

for idx,(x0,yd0) in enumerate(cases):
    x0c,yd0c=single_shooter(x0,yd0)
...
ax.plot(L1x,0,'r*',markersize=14,zorder=5,label='L1')
...
ax.set_title('L1 Lyapunov Orbit Family\nSingle Shooting — Earth-Moon CR3BP',fontsize=13)
ax.legend(fontsize=11)
ax.set_aspect('equal')
ax.grid(True,alpha=0.3)
plt.tight_layout()
plt.savefig('L1_family.png',dpi=150)
print("Saved L1_family.png")
